...
- month（1..12）
- org_id 或 org_cn
- product（auto/property/life/health）或 product_cn（车险/财产险/人身险/健康险）
- monthly_actual（万元）
...
1) 过滤器条（视角：全省/同城/异地；产品：汇总/车险/财产险/人身险/健康险；截至月份）
2) KPI 卡片区
   - 年度目标
   - 截至本月目标（YTD target）
   - 年度时间进度（线性/权重可切换）
   - 时间进度达成率（录入实际后点亮）
3) 图表 1：月度目标/实际/增长率（双 Y 轴）
   - 右轴增长率固定百分比格式，含 3% 预警线
4) 图表 2：季度目标（四块卡片或柱状）
5) 状态提示区（严格口径：哪些指标等待哪些数据）
...
1) 机构概况（年度目标、产品结构）
2) 月度/季度拆解表（含对账：合计回算年度）
3) 实际录入（导入或手动）
4) 达成率与时间进度达成率（点亮后显示双口径并列）
5) 增长与增量（点亮后显示）
