...
- [ ] **任务13**: 扩展数据结构（schema + 导入）
  - **位置**：`src/schemas/schema.ts`, `src/services/loaders.ts`, `src/app/import/page.tsx`
  - **内容**：
    1. 新增字段：`compulsory_premium`, `commercial_premium`
    2. 新增字段：`compulsory_expense_rate`, `commercial_expense_rate`
    3. 新增字段：`property_first_day_expense_rate`, `life_first_day_expense_rate`
  - **验证**：导入 CSV/JSON 能识别字段并通过校验

- [ ] **任务14**: 新增满期天数计算函数
  - **位置**：`src/domain`（新文件或复用现有）
  - **内容**：
    1. 实现“当月自然天数/2 + max(0, 统计日期 - 次月1日 + 1)”
    2. 统计日期=截至月份月末，计入当日
  - **验证**：覆盖跨月边界、当月内、超期场景
...
- [ ] **任务15**: 已赚保费计算函数
  ...
  - **内容**：
    ...
    2. 财产险：首日费用率公式
    3. 人身险：首日费用率 * 0.967
  - **验证**：与 `docs/business/指标定义规范.md` 公式一致

- [ ] **任务16**: 新增页面 `/earned-premium`
  - **位置**：`src/app/earned-premium/page.tsx`
  - **内容**：
    1. 过滤器：视角/机构/产品/截至月份/时间进度口径
    2. 图表：并列柱（保费收入/已赚保费）+ 折线（满期率）
    3. 车险保费收入使用商业险+交强险堆积
  - **验证**：切换产品、视角、月份均能更新图表

- [ ] **任务17**: 导航入口与文档提示
  - **位置**：`src/app/layout.tsx`, `docs/development/UI开发指南.md`
  - **内容**：新增导航与页面说明
  - **验证**：导航展示在“机构”之后

- [ ] **任务18**: 更新字段字典与索引
  - **位置**：`public/data/field_mapping.json`, `docs/.meta/*`
  - **内容**：补充新字段并同步索引
  - **验证**：docs-index/code-index/graph.json 对齐
