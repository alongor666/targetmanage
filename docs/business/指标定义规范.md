...
- 产品：product（auto/property/life/total）
...
> 说明：用于“已赚保费预测”页面，展示指定年度 1..截至月 的保费收入、已赚保费与满期率。

### 数据字段（实际/预测输入）

#### 月度实际（actuals_monthly）
- **compulsory_premium**：交强险保费收入（万元）
- **commercial_premium**：商业险保费收入（万元）
- **compulsory_expense_rate**：交强险首日费用率（0~1）
- **commercial_expense_rate**：商业险首日费用率（0~1）
- **property_first_day_expense_rate**：财产险首日费用率（0~1）
- **life_first_day_expense_rate**：人身险首日费用率（0~1）

#### 年度目标（targets_annual_2026）
- **compulsory_premium**：交强险年度目标保费（万元）
- **commercial_premium**：商业险年度目标保费（万元）
- **compulsory_expense_rate**：交强险首日费用率（0~1）
- **commercial_expense_rate**：商业险首日费用率（0~1）
- **property_first_day_expense_rate**：财产险首日费用率（0~1）
- **life_first_day_expense_rate**：人身险首日费用率（0~1）
...
> 统计日期 = 截至月份月末，计入当日；不封顶。

- **当月内部满期天数** = 当月自然天数 / 2
- **次月起满期天数** = max(0, 统计日期 − 次月1日 + 1)
- **当月保单到统计日期的满期天数** = 当月自然天数 / 2 + max(0, 统计日期 − 次月1日 + 1)

### 已赚保费公式

#### 车险（auto）
- 商业险已赚保费 = 商业险保费收入 × 费用率 × 0.94 + 商业险保费收入 × (1 - 费用率) × (满期天数 / 365)
- 交强险已赚保费 = 交强险保费收入 × 费用率 × 0.82 + 交强险保费收入 × (1 - 费用率) × (满期天数 / 365)
- 车险已赚保费 = 商业险已赚保费 + 交强险已赚保费

#### 财产险（property）
- 财产险已赚保费 = 财产险保费收入 × 首日费用率 + 财产险保费收入 × (1 - 首日费用率) × (满期天数 / 365)

#### 人身险（life）
- 人身险已赚保费 = 人身险保费收入 × 首日费用率 × 0.967 + 人身险保费收入 × (1 - 首日费用率) × (满期天数 / 365)


### 满期率
- **满期率** = 当月已赚保费 / 当月保费收入
- 车险当月保费收入 = 交强险保费收入 + 商业险保费收入
...
> 说明：基于三级机构的实际达成情况，预测四川分公司整体对总公司目标的完成度

### 核心概念
- **总公司目标**：总公司下达给四川分公司的年度保费目标（不按三级机构拆分，仅分产品）
- **三级机构计划**：四川分公司下达给各三级机构的保费计划（分机构、分产品）
- **预测逻辑**：基于三级机构的实际完成值聚合后，对比总公司目标，计算达成率

### 数据来源
- **总公司目标**：`public/data/headquarters_targets_annual_2026.json`
- **三级机构实际**：三级机构月度实际数据聚合到全省维度（`all` 视角）

### 预测指标

#### 年度预测达成率
- **hq_achievement_rate_annual** <!--impl:src/domain/headquarters.ts:17-22-->
  年度预测达成率 = 全省年累计实际 / 总公司年度目标

#### 月度累计预测达成率
- **hq_achievement_rate_monthly** <!--impl:src/domain/headquarters.ts:62-91-->
  月度累计达成率 = 全省月度累计实际 / 总公司月度累计目标
  - 月度累计目标 = 总公司年度目标 × (当前月份 / 12)

#### 季度累计预测达成率
- **hq_achievement_rate_quarterly** <!--impl:src/domain/headquarters.ts:99-104-->
  季度累计达成率 = 全省季度累计实际 / 总公司季度目标
  - 季度目标 = 总公司年度目标 × 0.25（简化：平均分配）

### 差距分析
- **hq_gap** <!--impl:src/domain/headquarters.ts:30-35-->
  差距值 = 实际完成值 - 总公司目标值
  - 正数：超额完成
  - 负数：未达标

### UI展示
- **分产品卡片**：显示各产品（车险、财产险、人身险、汇总）的总公司目标和当前达成率
- **月度累计图表**：展示选定产品的月度累计实际、累计目标、达成率趋势
- **颜色标识**：
  - 绿色：达成率 ≥ 100%
  - 红色：达成率 < 80%
  - 蓝色：80% ≤ 达成率 < 100%
...
### 使用场景
1. **目标差异监控**：当三级机构计划总和与总公司目标不一致时，监控实际达成情况
   - 当前情况：三级机构目标总和（73110万元）高于总公司目标（67650万元）约8.07%
   - 意味着：若所有三级机构达成目标，将超额完成总公司目标
2. **提前预警**：基于当前进度，预测年底是否能达成总公司目标
3. **产品结构调整**：识别哪些产品需要加强推动以达成总公司目标
4. **安全边际分析**：评估三级机构目标设定是否合理，留有多大的容错空间
