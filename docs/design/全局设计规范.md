...
:root {
    ...
    /* === 增长率预警配色（2025-12-23新增） === */
    --growth-warning-orange: #FF9500;       /* 预警时柱状图 - 橙色 */
    --growth-warning-dark-red: #8B0000;     /* 预警时标签 - 深红色 */
    --growth-default-gray: #6B7280;         /* 默认柱状图标签 - 灰色 */
    --growth-default-blue: #3B82F6;         /* 默认折线图标签 - 蓝色 */
    --growth-line-dark-blue: #1E40AF;       /* 折线图线条 - 深蓝色 */
    --growth-warning-threshold: 0.05;       /* 预警阈值 - 5% */

    /* === 年度对比数据颜色系统（2025-12-27新增） === */
    /* 渐变色序列：从灰到蓝的连续渐变，体现时间推进和确定性层次 */
    --chart-actual-2025: #BDC3C7;           /* 2025年实际 - 浅灰色（历史基线，渐变起点） */
    --chart-actual-2026: #7FB3D5;           /* 2026年实际 - 中蓝灰（当期数据，渐变过渡） */
    --chart-target-2026: #B0D8EF;           /* 2026年目标 - 浅蓝色（计划目标，渐变中段） */
    --chart-planned-2026: #E8F4FD;          /* 2026年规划 - 极浅蓝（预测规划，渐变终点） */
    --chart-planned-2026-border: #B0D8EF;   /* 2026年规划边框 - 浅蓝色（增强识别性） */
}
```
...
#### 增长率预警色彩（2025-12-23新增）
| 用途 | 颜色变量 | 十六进制 | 使用场景 |
|------|---------|---------|---------|
| 预警柱状图 | `--growth-warning-orange` | #FF9500 | 增长率 < 5% 时的柱状图颜色 |
| 预警标签文字 | `--growth-warning-dark-red` | #8B0000 | 预警状态的数值标签 |
| 默认柱状图标签 | `--growth-default-gray` | #6B7280 | 正常状态的柱状图标签 |
| 默认折线图标签 | `--growth-default-blue` | #3B82F6 | 正常状态的折线图标签 |
| 折线图线条 | `--growth-line-dark-blue` | #1E40AF | 折线图线条（保持不变） |
| 预警阈值 | `--growth-warning-threshold` | 0.05 | 5%预警阈值 |

#### 年度对比数据配色（2025-12-27新增）
| 数据类型 | 颜色变量 | 十六进制 | 渐变位置 | 设计语义 |
|---------|---------|---------|---------|---------|
| 2025年实际 | `--chart-actual-2025` | #BDC3C7 | 渐变起点 | 历史基线，浅灰色 |
| 2026年实际 | `--chart-actual-2026` | #7FB3D5 | 渐变过渡 | 当期数据，中蓝灰 |
| 2026年目标 | `--chart-target-2026` | #B0D8EF | 渐变中段 | 计划目标，浅蓝色 |
| 2026年规划 | `--chart-planned-2026` | #E8F4FD | 渐变终点 | 预测规划，极浅蓝 |
| 2026年规划边框 | `--chart-planned-2026-border` | #B0D8EF | - | 边框增强识别 |

**设计原理**：
- **渐变序列**：灰→蓝连续渐变，体现从历史到未来的时间推进
- **饱和度递减**：实际数据饱和度高，规划数据饱和度低，体现确定性层次
- **视觉过渡**：色相平滑过渡，减少视觉跳跃感
- **边框辅助**：规划数据使用边框增强识别性
...
### 8.3 机构映射表（2025-12-23更新）

#### 机构ID与名称对应关系
```javascript
/**
 * 机构ID到机构名称的映射关系
 * 基于实际数据文件 actuals_monthly_2025.json 中的机构ID
 */
const organizationMapping = {
    // === 同城机构 (local) ===
    'SC_local_benbu': '本部',           // 四川分公司本部
    'SC_local_tianfu': '天府',          // 天府新区机构
    'SC_local_gaoxin': '高新',          // 高新区机构
    'SC_local_xindu': '新都',           // 新都区机构
    'SC_local_qingyang': '青羊',        // 青羊区机构
    'SC_local_wuhou': '武侯',           // 武侯区机构
    'SC_local_xicai_junyuan': '西财俊苑', // 西财俊苑机构
    
    // === 异地机构 (remote) ===
    'SC_remote_yibin': '宜宾',          // 宜宾市机构
    'SC_remote_luzhou': '泸州',          // 泸州市机构
    'SC_remote_deyang': '德阳',          // 德阳市机构
    'SC_remote_ziyang': '资阳',          // 资阳市机构
    'SC_remote_leshan': '乐山',          // 乐山市机构
    'SC_remote_zigong': '自贡',          // 自贡市机构
    'SC_remote_dazhou': '达州'           // 达州市机构
};
...
const organizationGroups = {
    local: {
        name: '同城',
        description: '成都市区及周边机构',
        organizations: [
            'SC_local_benbu',           // 本部
            'SC_local_tianfu',          // 天府
            'SC_local_gaoxin',          // 高新
            'SC_local_xindu',           // 新都
            'SC_local_qingyang',        // 青羊
            'SC_local_wuhou',           // 武侯
            'SC_local_xicai_junyuan'    // 西财俊苑
        ],
        displayNames: ['本部', '天府', '高新', '新都', '青羊', '武侯', '西财俊苑']
    },
    remote: {
        name: '异地',
        description: '四川省内其他城市机构',
        organizations: [
            'SC_remote_yibin',          // 宜宾
            'SC_remote_luzhou',          // 泸州
            'SC_remote_deyang',          // 德阳
        ...
        displayNames: ['宜宾', '泸州', '德阳', '资阳', '乐山', '自贡', '达州']
    }
};
```

#### 机构命名规范说明
- **ID规范**: `SC_{group}_{name}`
  - `SC`: 四川省标识
  - `{group}`: `local`(同城) 或 `remote`(异地)
  - `{name}`: 机构拼音/英文标识
- **特殊机构**:
  - `SC_local_benbu`: 分公司本部，属于同城机构
  - `SC_local_xicai_junyuan`: 西财俊苑，特殊的同城机构

### 8.4 组织模式系统（2025-12-21新增）
```javascript
/**
 * 组织模式配置
...
const organizationModes = {
    ...
    local: {
        ...
        filter: (organizations) => organizations.filter(org => 
            ['本部', '天府', '高新', '新都', '青羊', '武侯', '西财俊苑'].includes(org)
        ),
        defaultSelection: ['本部', '天府', '高新', '新都', '青羊', '武侯', '西财俊苑']
    },
...
function detectOrganizationMode(selectedOrgs, allOrgs) {
    ...
    // 同城模式检查
    const localOrgs = ['本部', '天府', '高新', '新都', '青羊', '武侯', '西财俊苑'];
...
#### 增长率预警标签配置（2025-12-23新增）

使用 ECharts `rich` 文本样式实现动态标签颜色切换：

```javascript
// 颜色常量定义
const orangeColor = "#FF9500";           // 预警时柱状图
const darkRedColor = "#8B0000";          // 预警时标签
const grayColor = "#6B7280";             // 默认柱状图标签
const blueColor = "#3B82F6";             // 默认折线图标签
const darkBlueColor = "#1E40AF";         // 折线图线条
const warningThreshold = 0.05;           // 5%预警线

// 柱状图标签配置（带预警状态）
const barLabelWithWarning = {
    show: true,
    position: 'top',
    fontWeight: 'bold',
    overflow: 'truncate',
    ...
    rich: {
        normal: { color: grayColor },
        warning: { color: darkRedColor },
    },
};

// 折线图标签配置（带预警状态）
const lineLabelWithWarning = {
    show: true,
    position: 'top',
    fontWeight: 'bold',
    formatter: (params) => {
